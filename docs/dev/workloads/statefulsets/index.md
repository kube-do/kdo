...
与 [无状态应用(Deployment)](/dev/workloads/deployments/) 不同的是，`有状态应用(Statefulset)` 为每个 `容器组(Pod)` 提供了稳定的、唯一的网络标识符，并且保证有序的、优雅的部署和缩放。
...
{: .note }
有状态应用的访问页面基本和无状态应用一样，可以参考[访问无状态应用](/dev/workloads/deployments/#访问无状态应用)
...
{: .note }
操作有状态应用基本也和无状态应用一样，只是有些不适合有状态应用菜单没有显示，可以参考[操作无状态应用](/dev/workloads/deployments/#操作无状态应用)
