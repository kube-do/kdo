...
{: .note }
与其他容器云平台相比，KDO智能云原生平台在命令行功能上表现出色，提供了强大的支持。
它不仅兼容 `CloudShell`，还支持 `LocalShell`，为运维人员和开发人员带来了极大的便利和高效的操作体验。
无论是云端还是本地环境，KDO智能云原生平台都能满足多样化的需求，显著提升工作效率。
...
3. **安全性高：** 由于 `CloudShell` 是直接与您的KDO账户集成的，因此它继承了KDO智能云原生平台安全措施。这意味着您不需要担心密钥管理或SSH访问的问题。
...
## LocalShell模式
LocalShell主要用于促进本地环境与KDO智能云原生平台之间的交互操作。例如，它允许用户通过kubectl在本地对平台进行操作，以及实现本地IDE与KDO智能云原生平台之间的无缝连接。
这样的设置旨在提高开发效率和灵活性，使得开发者可以在熟悉的本地工具环境中高效工作，同时充分利用KDO智能云原生平台的强大功能。
...
### 操作localshell
点击下面`复制登录命令`，到终端运行后，就可以通过`oc`命令行工具访问KDO智能云原生平台(如果本机有`kubectl`，通过`oc`登录后用`kubectl`也是一样)。
